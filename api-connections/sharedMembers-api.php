...
function handleGet($pdo)
{
  /* =====================
     ...
     ===================== */
  $remoteUrl = "https://coletta-parecious-improperly.ngrok-free.dev/CampusWear/api/shared/isc.php";
  ...
  if ($response) {
    $remoteApplicants = json_decode($response, true);

    if (is_array($remoteApplicants)) {
      foreach ($remoteApplicants as $applicant) {

        // REQUIRED FIELDS CHECK
        if (!isset($applicant['firstName'], $applicant['lastName'], $applicant['email'])) {
          continue;
        }

        // 2. CHECK IF ALREADY EXISTS
        $check = $pdo->prepare(
          "SELECT COUNT(*) FROM tbl_members WHERE email = :email"
        );
        $check->execute([
        ...
        if ($check->fetchColumn() == 0) {
          ...
          $insert = $pdo->prepare(
            "INSERT INTO tbl_members 
             ...
             VALUES (:firstName, :lastName, :email, 'pending')"
          );
  ...
  /* =====================
     4. FETCH LOCAL MEMBERS
     ===================== */
  $stmt = $pdo->prepare("SELECT * FROM tbl_members ORDER BY mbID DESC");
  $stmt->execute();
