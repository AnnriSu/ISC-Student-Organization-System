...
function buildFullName($fname, $lname, $mname = '', $suffix = '') {
    $name = trim($fname);
    if (!empty($mname)) $name .= " " . trim($mname);
    $name .= " " . trim($lname);
                        ...
                        <tr>
                                ...
                                <select class="form-select form-select-sm status-select"
                                    ...
                                    <?php foreach ($statusOptions as $status): ?>
                                        <option value="<?= $status['apStatusID'] ?>"
                                            <?= 
                                                // Select Pending for new applications
                                                (strtolower($row['apStatusDesc']) === 'pending' && strtolower($status['apStatusDesc']) === 'pending') 
                                                || ($row['apStatusID'] == $status['apStatusID'] && strtolower($row['apStatusDesc']) !== 'pending') 
                                                ? 'selected' : '' ?>>
                            ...
                            <td class="action-cell">
                                <?php if (strtolower($row['apStatusDesc']) === 'approved'): ?>
                                    <button class="btn btn-success btn-sm send-approval" data-id="<?= $row['apID'] ?>">Send Membership Approval</button>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endwhile; ?>
...
// Attach existing approved buttons
